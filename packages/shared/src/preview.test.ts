...
describe("classifyPreview", () => {
  it.each(["a.jpg", "a.JPG", "a.jpeg", "a.png", "a.webp", "a.gif", "a.svg", "a.heic"])("%s は image", (name) => {
  ...
  it.each(["a.txt", "a.md", "a.json", "a.html", "a.ts", "a.py", "a.log"])("%s は text", (name) => {
    expect(classifyPreview(name)).toBe("text");
  });

  it.each(["a.zip", "a.pdf", "a.mkv", "README", "Makefile"])("%s は非対応（null）", (name) => {
