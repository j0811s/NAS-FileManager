...
- 表示形式: グリッド（画像はサムネイル、その他はアイコン）
- サムネイル取得: 既存の inline 配信 API（`api.previewUrl()`）をそのまま `<img src>` に使う。サーバ変更なし・依存追加なし。転送量は元画像そのままだが LAN 内運用なので許容
...
- 各カード:
  ...
  - サムネイル領域は `classifyPreview(name)`（`@nas-fm/shared`）で振り分け:
    ...
    - フォルダ → `Folder` アイコン、動画 → `Film` アイコン、その他 → `File` アイコン
...
- サムネイル `<img>` の読み込み失敗（削除直後・権限エラー等）: `onError` でファイルアイコン表示に差し替え。トーストは出さない（一覧表示の副次要素のため）
