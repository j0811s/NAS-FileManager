...
- **重要**: `heif-convert` は本セッションでは実バイナリでの動作確認ができていない（開発環境で `brew install libheif` の許可が得られなかったため）。デプロイ後は実際の iPhone 撮影 HEIC ファイルを1枚アップロードし、一覧サムネイルとプレビューモーダルの両方が正しく表示されることを必ず確認すること
