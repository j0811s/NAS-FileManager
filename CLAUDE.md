...
プロダクト仕様は `docs/spec.md` を参照（API 設計・セキュリティ必須事項・デプロイ・プレビュー機能など。機能実装の前に該当セクションを読むこと）。開発の進行状況とフェーズ分けは `docs/roadmap.md`。TypeScript 制約と features 構成のルールは `.claude/rules/`（typescript.md / features.md）にあり、該当パスのファイルを扱うとき自動で読み込まれる。
